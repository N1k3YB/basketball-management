...
export default function StatsPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const teamsResponse = await fetch('/api/stats/teams');
      if (!teamsResponse.ok) {
  ...
  const navigateToPlayerProfile = (playerId: string, userId?: string) => {
    // Если доступен userId, используем его для навигации к профилю пользователя
    // В противном случае используем id игрока (для обратной совместимости)
    router.push(`/profile/${userId || playerId}`);
  };

  // Навигация к профилю команды
  ...
  const exportToPDF = async () => {
    try {
      ...
      for (let pdfPage = 0; pdfPage < totalPdfPages; pdfPage++) {
        // Добавление новой страницы, если не первая
        if (pdfPage > 0) {
          pdf.addPage();
        }
      ...
      <div className="flex flex-col space-y-6">
        ...
        {loading || exporting ? (
          <div className="min-h-[60vh] flex items-center justify-center">
            <BasketballSpinner label={exporting ? 'Создание экспорта...' : 'Загрузка данных...'} />
          </div>
        ) : error ? (
              ...
              <div className="mb-4">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setPlayerPage(1); // Сбрасываем на первую страницу при изменении поиска
                  }}
              ...
              <div className="mb-4">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setTeamPage(1); // Сбрасываем на первую страницу при изменении поиска
                  }}
                  className="w-full p-2 border border-gray-300 rounded-md"
                />
              </div>
